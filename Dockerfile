...
RUN apt-get update && apt-get install -y  git-core libcurl4-openssl-dev libgit2-dev libssh2-1-dev libssl-dev libxml2-dev libpython3-dev python3-pip libpng-dev make pandoc pandoc-citeproc zlib1g-dev && rm -rf /var/lib/apt/lists/*
...
RUN Rscript -e 'remotes::install_version("reticulate",upgrade="never", version = "1.16")'
